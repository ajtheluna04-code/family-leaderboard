...
export default async function Home({
  ...
  const selectedGame =
    (games ?? []).find((g) => g.slug === selectedSlug) ?? (games ?? [])[0];
  const gameId = selectedGame?.id;

  // ✅ THIS is the line you accidentally put inside the page before.
  // It belongs here (in JS), not inside the returned JSX.
    ...
    <>
      <div className="header">
        <div>
          ...
          <div className="small">
            Public view • Family members log in to submit results.
          </div>
        </div>
